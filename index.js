...
// Comando expand (stub para desenvolvimento futuro)
program
  .command('expand')
  .description('Expande uma tarefa em subtarefas')
  .argument('<id>', 'ID da tarefa')
  .option('--num <number>', 'Número de subtarefas a serem criadas', '3')
  .action((id, options) => {
    console.log(chalk.yellow(`Funcionalidade de expansão para tarefa ${id} será implementada em breve!`));
  });
